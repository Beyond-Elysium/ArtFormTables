...
export interface SyncDbClient {
  ...
  createCompetitorActivity(data: CompetitorActivityCreateInput): Promise<void>;
}
  ...
  for (const agency of config.trackedAgencies) {
    ...
    for (const naicsCode of naicsCodes) {
      ...
      for (const raw of awards) {
        ...
        if (!db) {
          // TODO(db): Prisma client not wired in yet — log instead of writing.
          console.log("[vantage sync] TODO(db): would create competitor activity", row.awardId);
        } else {
          await db.createCompetitorActivity(row);
        }
        competitorAwards += 1;
      }
    }
